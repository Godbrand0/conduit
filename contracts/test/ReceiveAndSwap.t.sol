...
contract ReceiveAndSwapTest is Test {
    ...
    function test_malformedHookTarget_refundsToMessageSender() public {
        // Unknown selector on the executor → hook rejected → no recipient can
        // be read out of a 4-byte calldata, so the last-resort refund goes to
        // messageSender
    ...
    /// points. Anyone can originate a CCTP burn naming this contract as
    ...
    function test_foreignHookTarget_isRejectedAndRefunded() public {
        ...
        // Try to make the executor call USDC.transfer(attacker, ...) as itself.
